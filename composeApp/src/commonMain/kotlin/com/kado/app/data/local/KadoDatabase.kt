package com.kado.app.data.local

import androidx.room.ConstructedBy
import androidx.room.Database
import androidx.room.RoomDatabase
import androidx.room.RoomDatabaseConstructor
import androidx.room.migration.Migration
import androidx.sqlite.SQLiteConnection
import androidx.sqlite.execSQL
import com.kado.app.data.local.dao.BulkEditRuleDao
import com.kado.app.data.local.dao.CardDao
import com.kado.app.data.local.dao.CardStateDao
import com.kado.app.data.local.dao.DeckDao
import com.kado.app.data.local.dao.PatchDiffDao
import com.kado.app.data.local.dao.ReviewHistoryDao
import com.kado.app.data.local.dao.SettingsDao
import com.kado.app.data.local.entity.BulkEditRuleEntity
import com.kado.app.data.local.entity.CardEntity
import com.kado.app.data.local.entity.CardStateEntity
import com.kado.app.data.local.entity.DeckEntity
import com.kado.app.data.local.entity.PatchDiffEntryEntity
import com.kado.app.data.local.entity.ReviewHistoryEntity
import com.kado.app.data.local.entity.SettingsEntity

val MIGRATION_1_2 = object : Migration(1, 2) {
    override fun migrate(connection: SQLiteConnection) {
        connection.execSQL(
            "CREATE TABLE IF NOT EXISTS `settings` (" +
                "`id` INTEGER NOT NULL PRIMARY KEY, " +
                "`themeMode` TEXT NOT NULL DEFAULT 'system', " +
                "`cardFontScale` REAL NOT NULL DEFAULT 1.0, " +
                "`appFontScale` REAL NOT NULL DEFAULT 1.0)"
        )
    }
}

val MIGRATION_2_3 = object : Migration(2, 3) {
    override fun migrate(connection: SQLiteConnection) {
        connection.execSQL("ALTER TABLE cards ADD COLUMN subDeckIndex INTEGER DEFAULT NULL")
        connection.execSQL("CREATE INDEX IF NOT EXISTS index_cards_deckId_subDeckIndex ON cards (deckId, subDeckIndex)")
    }
}

val MIGRATION_3_4 = object : Migration(3, 4) {
    override fun migrate(connection: SQLiteConnection) {
        connection.execSQL("ALTER TABLE settings ADD COLUMN language TEXT NOT NULL DEFAULT 'en'")
    }
}

val MIGRATION_4_5 = object : Migration(4, 5) {
    override fun migrate(connection: SQLiteConnection) {
        connection.execSQL(
            "CREATE TABLE IF NOT EXISTS `bulk_edit_rules` (" +
                "`id` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, " +
                "`name` TEXT NOT NULL, " +
                "`frontFindPattern` TEXT NOT NULL, " +
                "`frontReplacePattern` TEXT NOT NULL, " +
                "`frontIsRegex` INTEGER NOT NULL DEFAULT 0, " +
                "`frontEnabled` INTEGER NOT NULL DEFAULT 1, " +
                "`backFindPattern` TEXT NOT NULL, " +
                "`backReplacePattern` TEXT NOT NULL, " +
                "`backIsRegex` INTEGER NOT NULL DEFAULT 0, " +
                "`backEnabled` INTEGER NOT NULL DEFAULT 1, " +
                "`createdAt` INTEGER NOT NULL)"
        )
    }
}

val MIGRATION_5_6 = object : Migration(5, 6) {
    override fun migrate(connection: SQLiteConnection) {
        connection.execSQL("ALTER TABLE card_states ADD COLUMN stability REAL DEFAULT NULL")
        connection.execSQL("ALTER TABLE card_states ADD COLUMN difficulty REAL DEFAULT NULL")
        connection.execSQL("ALTER TABLE card_states ADD COLUMN fsrsState INTEGER DEFAULT NULL")
        connection.execSQL("ALTER TABLE card_states ADD COLUMN step INTEGER DEFAULT NULL")
        connection.execSQL("ALTER TABLE card_states ADD COLUMN lastReview INTEGER DEFAULT NULL")
        connection.execSQL("ALTER TABLE decks ADD COLUMN schedulerType TEXT NOT NULL DEFAULT 'SM2'")
        connection.execSQL("ALTER TABLE decks ADD COLUMN fsrsParams TEXT DEFAULT NULL")
    }
}

val MIGRATION_6_7 = object : Migration(6, 7) {
    override fun migrate(connection: SQLiteConnection) {
        connection.execSQL(
            "CREATE TABLE IF NOT EXISTS `review_history` (" +
                "`id` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, " +
                "`cardId` INTEGER NOT NULL, " +
                "`deckId` INTEGER NOT NULL, " +
                "`reviewedAt` INTEGER NOT NULL, " +
                "`rating` INTEGER NOT NULL, " +
                "`durationMs` INTEGER NOT NULL, " +
                "`previousInterval` INTEGER NOT NULL, " +
                "`newInterval` INTEGER NOT NULL, " +
                "`previousQueue` INTEGER NOT NULL, " +
                "`newQueue` INTEGER NOT NULL, " +
                "FOREIGN KEY(`deckId`) REFERENCES `decks`(`id`) ON UPDATE NO ACTION ON DELETE CASCADE, " +
                "FOREIGN KEY(`cardId`) REFERENCES `cards`(`id`) ON UPDATE NO ACTION ON DELETE CASCADE" +
                ")"
        )
        connection.execSQL("CREATE INDEX IF NOT EXISTS `index_review_history_deckId` ON `review_history` (`deckId`)")
        connection.execSQL("CREATE INDEX IF NOT EXISTS `index_review_history_cardId` ON `review_history` (`cardId`)")
        connection.execSQL(
            "CREATE INDEX IF NOT EXISTS `index_review_history_reviewedAt` ON `review_history` (`reviewedAt`)"
        )
        connection.execSQL(
            "CREATE INDEX IF NOT EXISTS `index_review_history_deckId_reviewedAt` " +
                "ON `review_history` (`deckId`, `reviewedAt`)"
        )
    }
}

val MIGRATION_7_8 = object : Migration(7, 8) {
    override fun migrate(connection: SQLiteConnection) {
        connection.execSQL("ALTER TABLE cards ADD COLUMN ankiGuid TEXT DEFAULT NULL")
        connection.execSQL(
            "CREATE INDEX IF NOT EXISTS index_cards_deckId_ankiGuid ON cards (deckId, ankiGuid)"
        )
        connection.execSQL(
            "CREATE TABLE IF NOT EXISTS `patch_diff_entries` (" +
                "`id` INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, " +
                "`sessionId` TEXT NOT NULL, " +
                "`kind` INTEGER NOT NULL, " +
                "`existingCardId` INTEGER, " +
                "`newAnkiGuid` TEXT, " +
                "`newFront` TEXT, " +
                "`newBack` TEXT, " +
                "`createdAt` INTEGER NOT NULL)"
        )
        connection.execSQL(
            "CREATE INDEX IF NOT EXISTS `index_patch_diff_entries_session_kind` " +
                "ON `patch_diff_entries` (`sessionId`, `kind`)"
        )
    }
}

@Database(
    entities = [
        DeckEntity::class, CardEntity::class, CardStateEntity::class,
        SettingsEntity::class, BulkEditRuleEntity::class, ReviewHistoryEntity::class,
        PatchDiffEntryEntity::class
    ],
    version = 8,
    exportSchema = false
)
@ConstructedBy(KadoDatabaseConstructor::class)
abstract class KadoDatabase : RoomDatabase() {
    abstract fun deckDao(): DeckDao
    abstract fun cardDao(): CardDao
    abstract fun cardStateDao(): CardStateDao
    abstract fun settingsDao(): SettingsDao
    abstract fun bulkEditRuleDao(): BulkEditRuleDao
    abstract fun reviewHistoryDao(): ReviewHistoryDao
    abstract fun patchDiffDao(): PatchDiffDao
}

// The actual implementation is generated by Room's KSP processor
@Suppress("NO_ACTUAL_FOR_EXPECT")
expect object KadoDatabaseConstructor : RoomDatabaseConstructor<KadoDatabase>
