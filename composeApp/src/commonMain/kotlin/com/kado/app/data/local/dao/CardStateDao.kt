...
interface CardStateDao {
    ...
    @Query(
        "SELECT COUNT(*) FROM card_states WHERE cardId IN (SELECT id FROM cards WHERE deckId = :deckId) AND queue = 0"
    )
    suspend fun countNewByDeckId(deckId: Long): Int

    @Query(
        "SELECT COUNT(*) FROM card_states WHERE cardId IN (SELECT id FROM cards WHERE deckId = :deckId) AND (queue = 0 OR due <= :now)"
    )
    suspend fun countDueByDeckId(deckId: Long, now: Long): Int

    @Query(
        "SELECT COUNT(*) FROM card_states WHERE cardId IN (SELECT id FROM cards WHERE deckId = :deckId) AND queue = 1"
    )
    suspend fun countLearningByDeckId(deckId: Long): Int

    @Query(
        "SELECT COUNT(*) FROM card_states WHERE cardId IN (SELECT id FROM cards WHERE deckId = :deckId) AND queue = 2 AND due <= :now"
    )
    ...
    @Query(
        "SELECT COUNT(*) FROM card_states WHERE cardId IN (SELECT id FROM cards WHERE deckId = :deckId AND subDeckIndex = :subDeckIndex) AND queue = 0"
    )
    suspend fun countNewByDeckIdAndSubDeck(deckId: Long, subDeckIndex: Int): Int

    @Query(
        "SELECT COUNT(*) FROM card_states WHERE cardId IN (SELECT id FROM cards WHERE deckId = :deckId AND subDeckIndex = :subDeckIndex) AND (queue = 0 OR due <= :now)"
    )
    suspend fun countDueByDeckIdAndSubDeck(deckId: Long, subDeckIndex: Int, now: Long): Int

    @Query(
        "SELECT * FROM card_states WHERE cardId IN (SELECT id FROM cards WHERE deckId = :deckId AND subDeckIndex = :subDeckIndex)"
    )
    ...
    suspend fun getNextLearningByDeckIdAndSubDeck(
        deckId: Long,
        subDeckIndex: Int,
        now: Long,
    ...
    suspend fun getNextReviewByDeckIdAndSubDeck(
        deckId: Long,
        subDeckIndex: Int,
        now: Long,
        excludeCardId: Long = -1
    ...
    suspend fun getNextNewByDeckIdAndSubDeck(
        deckId: Long,
        subDeckIndex: Int,
        excludeCardId: Long = -1
    ): CardStateEntity?
